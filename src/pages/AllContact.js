...
const AllContact = () => {
  ...
  useEffect(async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://cms-backend-f2if.onrender.com/api/mycontacts`, {
        method: "GET",
        headers: {
  ...
  const deleteContact = async (id) => {
    if (window.confirm("are you sure you want to delete this contact ?")) {
      try {
        const res = await fetch(`https://cms-backend-f2if.onrender.com/api/delete/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    <>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>
          ...
          <p>
            <strong>Company</strong>: {modalData.address}
          </p>
          <p>
